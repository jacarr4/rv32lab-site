...
function enable(on){['reset','previous','play','next','next-instruction','scrubber','program','backend'].forEach(id=>$(id).disabled=!on)}
...
function apply(data,program,captureSource){if(!data?.frames?.length){session=null;enable(false);results(data.summary||data);listing({listing:data.listing||data.program?.listing||[]});$('message').className='message error';$('message').textContent=data.summary?.trap?.cause||data.trap?.cause||data.summary?.stop_reason||'Execution stopped before a trace frame was produced.';$('program').disabled=false;$('backend').disabled=false;return}componentDetailController?.abort();componentDetailSession++;componentDetailCache=new Map();componentDetail=null;componentDetailError=null;componentDetailProgram=program;componentDetailSource=captureSource;session=data;index=0;selectedAdder=null;selectedComponentGate=null;selectedTopologyNode=null;selectedGateNode=null;selectedGateOutput=null;selectedGateOperation=null;selectedInspectorComponent=null;selectedInspectorNand=null;genericGraph=null;expandedChains.clear();pause();listing({listing:data.listing||data.program?.listing||[]});$('source-file').textContent=data.program?.title||data.program?.id||`${program}.s`;$('backend').value=data.summary?.backend||$('backend').value;$('scrubber').max=data.frames.length-1;enable(true);if(staticManifest)$('backend').disabled=true;results(data.summary);const warn=data.warning||data.summary?.warning;$('message').className=`message${warn?' error':''}`;$('message').textContent=warn||`${data.summary?.instructions??'Trace'} instructions · ${data.summary?.cycles??data.frames.length} cycles · ${data.summary?.backend??'RV32I'} datapath`;render()}
async function requestRamDetail(component) {
  ...
  const request = ++componentDetailRequest, detailSession = componentDetailSession, detailController = componentDetailController, backend = $('backend').value, program = componentDetailProgram;
  try {
    let response;
...
async function loadStatic(program) {
  ...
  try {
    ...
    if (token === requestId) { session=null; enable(false); $('program').disabled=false; $('backend').disabled=true; }
    throw error;
  }
}
async function bootstrap() {
  ...
  try {
    ...
    if (response.status === 404) { load('sum'); return; }
    if (!response.ok) throw Error(`HTTP ${response.status}`);
  ...
  $('backend').disabled = true; $('editor-panel').hidden = true;
