...
function isInstructionStart(frame){const phase=String(frame?.phase||'').toUpperCase();if(phase.startsWith('FETCH'))return true;if(['DECODE','EXECUTE','MEMORY','WRITEBACK'].some(known=>phase.startsWith(known)))return false;return Boolean(frame?.instruction_start)}
async function moveInstruction(step=1){if(!session)return;pause();const token=++navigationId,activeSession=session,start=index+step;let target=-1;try{if(staticManifest)target=await findStaticFrame(isInstructionStart,start,step);else for(let position=start;position>=0&&position<session.frames.length;position+=step){if(isInstructionStart(session.frames[position])){target=position;break}}}catch(error){if(token===navigationId&&activeSession===session){$('message').className='message error';$('message').textContent=`Saved trace frame could not be loaded: ${error.message}`}return}if(token!==navigationId||activeSession!==session||target<0)return;navigate(target)}
...
function listing(program){const root=$('listing');root.replaceChildren();rowsByLine=new Map();rowsByAddress=new Map();const f=document.createDocumentFragment();for(const item of program.listing||[]){const row=document.createElement('div'),k=type(item);row.className=`source-row ${k}`;if(item.line!=null){row.dataset.line=item.line;rowsByLine.set(+item.line,row)}if(item.address!=null){row.dataset.address=item.address;if(!rowsByAddress.has(+item.address))rowsByAddress.set(+item.address,row)}const addr=document.createElement('span'),text=document.createElement('span');addr.className='source-address';text.className='source-text';addr.textContent=item.address==null?'':hex(item.address);text.textContent=item.text||' ';row.append(addr,text);if(k==='instruction'&&item.address!=null){row.tabIndex=0;row.setAttribute('role','button');const jump=async()=>{const matching=frame=>+frame.instruction_address===+item.address;const next=staticManifest?await findStaticFrame((frame,position)=>position>index&&isInstructionStart(frame)&&matching(frame),index+1):session.frames.findIndex((frame,position)=>position>index&&isInstructionStart(frame)&&matching(frame));const fallback=next<0?(staticManifest?await findStaticFrame(matching,0):session.frames.findIndex(matching)):next;pause();navigate(fallback<0?0:fallback)};row.onclick=jump;row.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();jump()}}}f.append(row)}root.append(f)}
